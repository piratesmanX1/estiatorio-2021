...
export default function AdminUserPending() {
    ...
    const EnhancedTableToolbar = (props) => {
        ...
        const editData = () => {
            const API_PATH = `http://localhost:80/APU/FYP/estiatorio/src/api/status_pending.php`; // direct it to the PHP folder
            ...
            if (tableData) {
                ...
                tableData.forEach(function(item) {
                    ...
                    .then((res) => {
                        ...
                        if (res.data == true) {
                            // if data updated
                            enqueueSnackbar('Staff account ID: ' + item.id + ' approved and activated.', {variant: 'success'});
                            // refresh the data
                            fetchData();
        ...
        const deleteData = () => {
            const API_PATH = `http://localhost:80/APU/FYP/estiatorio/src/api/status_disapprove.php`; // direct it to the PHP folder
            ...
            if (tableData) {
                ...
                tableData.forEach(function(item) {
                    ...
                    .then((res) => {
                        ...
                        if (res.data == true) {
                            // if data updated
                            enqueueSnackbar('Staff account ID: ' + item.id + ' disapproved and deleted.', {variant: 'success'});
                            // refresh the data
                            fetchData();
                            setLoadingOpen(false);
                        } else {
        ...
        return (
            ...
            {numSelected > 0 ? (
                ...
                <Typography className={classes.title} variant="h6" id="tableTitle" component="div">
                    ...
                    <Typography color="textSecondary" variant="body1">
                        * Staff account's registration that being rejected will be immediately deleted from the database.
                    </Typography>
                </Typography>
            )}
            ...
            {numSelected > 0 ? (
                <>
                    ...
                    >
                        <DialogTitle id="responsive-dialog-title">{"Are you sure you wish to disapprove the selected staff accounts?"}</DialogTitle>
                        <DialogContent>
                            <DialogContentText>
                                This action is irreversible and disapproved accounts will be deleted from the database.
                            </DialogContentText>
                        </DialogContent>
                        <DialogActions>
                    ...
                    <Tooltip title="Approve staff">
                        <IconButton onClick={editData} className="data-theader-button" aria-label="edit data">
                            <DoneAllOutlinedIcon />
                        </IconButton>
                    </Tooltip>
                </>
    ...
    const fetchData = () => {
        const API_PATH = `http://localhost:80/APU/FYP/estiatorio/src/api/fetch_pending.php`; // direct it to the PHP folder
