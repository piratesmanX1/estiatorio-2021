...
export default function AdminUserPending() {
    function createData(id, name, reservation_time, email, phone_number, status) {
        return {id, name, reservation_time, email, phone_number, status};
      }
    ...
    { id: 'reservation_time', numeric: true, disablePadding: false, label: 'Reservation Time' },
    { id: 'email', numeric: true, disablePadding: false, label: 'Email' },
    { id: 'phone_number', numeric: true, disablePadding: false, label: 'Phone No.' },
    ...
    const EnhancedTableToolbar = (props) => {
        ...
        const updateData = (update_status) => {
            const API_PATH = `http://localhost:80/APU/FYP/estiatorio/src/api/status_reservation_list.php`; // direct it to the PHP folder

            const data_type = "Reservation Log";
            ...
            if (tableData) {
                ...
                tableData.forEach(function(item) {
                    // if the reservation is already cancelled, it cant be changed anymore
                    if (item.status == update_status) {
                        enqueueSnackbar(data_type + " ID: " + item.id + " is already in the status you intended to update.", {variant: 'error'});
                    } else if (item.status == 2) {
                        enqueueSnackbar(data_type + " ID: " + item.id + " is already cancelled, you can't change the status anymore.", {variant: 'error'});
                    } else {
                        setLoadingOpen(true);
                        // if its not then allow it to be changed
                        ...
                        .then((res) => {
                            ...
                            if (res.data == true) {
                                // if data updated
                                enqueueSnackbar('Reservation Log ID: ' + item.id + ' status updated.', {variant: 'success'});
                                // refresh the data
                                fetchData();
                        ...
                        .catch((err) =>  {
                            ...
                            // refresh the data
                            fetchData();
                            throw Error("Error: Reservation log update process on " + data_type + " ID: " + item.id + " failed for some reason. Error: " + err); // making a custom error message that will show in console
                        });
                    }
                });
                ...
                enqueueSnackbar("Error: Reservation log data is not defined, please re-check the table row again.", {variant: 'error'});
            }
        }
            ...
            {numSelected > 0 ? (
                ...
                <Typography className={classes.title} variant="h6" id="tableTitle" component="div">
                    ...
                    <Typography color="textSecondary" variant="body1">
                        * Reservation list will be automatically cancelled by the system if the reservation time is overdue.
                    </Typography>
                </Typography>
            )}
            ...
            {numSelected > 0 ? (
                <>
                    <Tooltip title="Mark as Attended">
                        <IconButton onClick={() => updateData(0)} className="data-theader-button" aria-label="delete">
                            <ExploreIcon/>
                        </IconButton>
                    </Tooltip>
                    ...
                    >
                        <DialogTitle id="responsive-dialog-title">{"Are you sure you wish to cancel the selected reservations?"}</DialogTitle>
                        <DialogContent>
                            <DialogContentText>
                                This action is irreversible and reservation will be cancelled.
                            </DialogContentText>
                        </DialogContent>
                        <DialogActions>
                        ...
                        <Button onClick={() => updateData(2)} color="primary" autoFocus>
                            Proceed
                        </Button>
                        </DialogActions>
                    </Dialog>
    ...
    const fetchReservationList = (restaurant_id) => {
        const API_PATH = `http://localhost:80/APU/FYP/estiatorio/src/api/fetch_reservation_list.php`; // direct it to the PHP folder
            ...
            .then((res) => {
                ...
                if (res.data == false) {
                    ...
                    res.data.map((data) => {
                        ...
                        datarows.push(createData(data.reservation_id, fullName, reservation_time, data.email, data.phone_number, data.status));
                    });  
        ...
        axios.post(API_PATH, formData) // asynchronous, therefore promises
            .then((res) => {
                ...
                if (res.data == false) {
                    ...
                    let datarows;
                    res.data.map((data) => {
                        // then call in the related reservation list of the restaurant
                        fetchReservationList(data.restaurant_id);
                    });
                } else {
                    // in this case res.data must be returning an error message
                    enqueueSnackbar(res.data, {variant: 'error'});
                            ...
                            {stableSort(rows, getComparator(order, orderBy))
                                ...
                                return (
                                    ...
                                    <TableCell component="th" id={labelId} scope="row" padding="none">
                                        ...
                                        <TableCell align="right" style={row.status == 0 ? {color: "green"} : row.status == 1 ? {color: "#FFD800"} : {color: "red"}}>{row.status == 0 ? "Attended" : row.status == 1 ? "Ongoing" : "Cancelled"}</TableCell>
                                    </TableRow>
                                );
                                })}
                            {emptyRows > 0 && (
